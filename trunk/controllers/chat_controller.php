...
class ChatController extends AppController {
    ...
    function index() {
    	...
    	$chat_messages = $this->ChatMessage->findAll(array('facilitated_class_id' => $this->viewVars['facilitated_class']['id']),null,'ChatMessage.created DESC',30);
    ...
    function leave() {
    	$this->ChatParticipant->query('delete from chat_participants where user_id = ' . $this->viewVars['user']['id'] . ' AND facilitated_class_id = ' . $this->viewVars['facilitated_class']['id'] . ';');
    	exit;
    }
    ...
    function send() {
    	...
    	$data = array('ChatMessage' => array(
    		...
    		'facilitated_class_id' => $this->viewVars['facilitated_class']['id'], 
    ...
    function get($datetime) {
    	...
    	$chat_messages = $this->ChatMessage->findAll(
    		array(
				'ChatMessage.facilitated_class_id' => $this->viewVars['facilitated_class']['id'],
    ...
    function updateChatParticipants() {
		...
		$chat_participant = $this->ChatParticipant->find(array('user_id'=>$this->viewVars['user']['id'],'facilitated_class_id'=>$this->viewVars['facilitated_class']['id']),array('id'));
		if(isset($chat_participant['ChatParticipant']['id'])) {
			...
			$chat_participant = array('ChatParticipant' => array('user_id'=>$this->viewVars['user']['id'],'facilitated_class_id'=>$this->viewVars['facilitated_class']['id'],'modified'=>date('Y-m-d H:i:s')));
			$this->ChatParticipant->save($chat_participant);
		}

		$this->ChatParticipant->contain(array('User.fields' => array('first_name','last_name')));
		$chat_participants = $this->ChatParticipant->findAll(array('facilitated_class_id'=>$this->viewVars['facilitated_class']['id']));
