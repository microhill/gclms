...
var AES = {
	...
	Cipher: function (input, key, w) {    // main Cipher function [�5.1]
	...
	Encrypt: function (plaintext, password, nBits) {
	  ...
	  plaintext = AES.encodeUTF8(plaintext);  // so that text is single-byte characters only
	
	  // for this example script, generate the key by applying Cipher to 1st 16/24/32 chars of password; 
	  // for real-world applications, a more secure approach would be to hash the password e.g. with SHA-1
	  var nBytes = nBits/8;  // no bytes in key
	  var pwBytes = new Array(nBytes);
	  for (var i=0; i<nBytes; i++) pwBytes[i] = AES.encodeUTF8(password).charCodeAt(i);
	  var pwKeySchedule = AES.KeyExpansion(pwBytes);
	  var key = AES.Cipher(pwBytes, pwBytes, pwKeySchedule);
	  ...
	  for (var b=0; b<blockCount; b++) {
	    ...
	    var cipherCntr = AES.Cipher(counterBlock, key, keySchedule);  // -- encrypt counter block --
	    ...
	    ciphertext[b] = AES.escCtrlChars(ct);  // escape troublesome characters in ciphertext
	  }
	
	  // convert the nonce to a string to go on the front of the ciphertext
	  var ctrTxt = '';
	  for (var i=0; i<8; i++) ctrTxt += String.fromCharCode(counterBlock[i]);
	  ctrTxt = AES.escCtrlChars(ctrTxt);
	...
	Decrypt: function (ciphertext, password, nBits) {
	  ...
	  var ctrTxt = AES.unescCtrlChars(ciphertext[0]);
	  ...
	  for (var b=1; b<ciphertext.length; b++) {
	    ...
	    ciphertext[b] = AES.unescCtrlChars(ciphertext[b]);
	  ...
	  return AES.decodeUTF8(plaintext.join(''));
	},
	
	encodeUTF8: function (str) {  // encode multi-byte string into utf-8 multiple single-byte characters 
	  str = str.replace(
	...
	decodeUTF8: function (str) {  // decode utf-8 encoded string back into multi-byte characters
	  str = str.replace(
	      /[\u00c0-\u00df][\u0080-\u00bf]/g,                 // 2-byte chars
	      function(c) { 
...
test = AES.Encrypt(phrase, password, 256);
alert(AES.Decrypt(test,password,256));