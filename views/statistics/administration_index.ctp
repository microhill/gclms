...
<div class="gclms-center-column">
	<div class="gclms-content">
		...
		<h2>New Users</h2>
		<p><img src="http://chart.apis.google.com/chart?cht=lc&chxt=x,y&chds=<?= min($users_created) ?>,<?= max($users_created) ?>&chf=bg,s,f6f6f6&chd=t:<?= implode(',',$users_created) ?>&chs=600x200" /></p>
		
		<h2>Forum Posts</h2>
		<p><img src="http://chart.apis.google.com/chart?cht=lc&chxt=x,y&chds=<?= min($forum_posts_created) ?>,<?= max($forum_posts_created) ?>&chf=bg,s,f6f6f6&chd=t:<?= implode(',',$forum_posts_created) ?>&chs=600x200" /></p>
	</div>
</div>
